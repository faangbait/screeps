use log::warn;
use screeps::{Room, HasPosition, RoomPosition, Position, RoomName};
        ...
        Some(r) => {
            match r.get_position_at(pos.x(), pos.y()) {
                Some(pos) => {
                    ...
                    if screeps::game::flags::get(&name).is_some() { return; }
                    ...
                    match r.create_flag(&pos, &name, screeps::Color::Green, screeps::Color::Green) {
                        ...
                        Err(e) => match e {
                            screeps::ReturnCode::NameExists => warn!("NAME EXISTS:  {:?} {:?}", name, pos),
                            screeps::ReturnCode::Full => warn!("TOO MANY FLAGS:  {:?} {:?}", name, pos),
                            screeps::ReturnCode::InvalidArgs => warn!("INVALID ARGS:  {:?} {:?}", name, pos),
                            _ => warn!("Unhandled return code in flag"),
                        },
                    }
                },
